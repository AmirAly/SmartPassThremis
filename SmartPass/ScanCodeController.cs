...
namespace SmartPass
{
	public class ContentView : UIView
	{
		...
		public ContentView(UIColor fillColor, AVCaptureVideoPreviewLayer layer, MyMetadataOutputDelegate metadataSource)
		{
			...
			float TopMargin = (float)this.Frame.Height / 3;
			float SideMargin = (float)this.Frame.Width / 6;
			this.layer = layer;
			...
			var label = new UILabel(new RectangleF(TopMargin-60, SideMargin,(float)(this.Frame.Width - (SideMargin*2)),50));
			label.Text = "Please center the code in the middle of te square";
			AddSubview(label);
			var lblSq = new UILabel(new RectangleF(TopMargin, SideMargin, (float)(this.Frame.Width - (SideMargin * 2)), TopMargin));
			lblSq.Text = "";
